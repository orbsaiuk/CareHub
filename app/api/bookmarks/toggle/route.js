...
import { SUPPLIER_BY_TENANTID_OR_SLUG_QUERY } from "@/sanity/queries/supplier";
...
export async function POST(req) {
  ...
  try {
    ...
    // Try to find as company first, then as supplier
    let tenant = await writeClient.fetch(COMPANY_BY_TENANTID_OR_SLUG_QUERY, {
      id: targetId,
    });

    if (!tenant?._id) {
      tenant = await writeClient.fetch(SUPPLIER_BY_TENANTID_OR_SLUG_QUERY, {
        id: targetId,
      });
    }

    if (!tenant?._id)
      return new Response("Company or Supplier not found", { status: 404 });
    ...
    if (bookmarkExists) {
      // REMOVE: Get all bookmarks except the ones referencing this tenant
      const filteredBookmarks = (fullUser.bookmarks || []).filter(
        (bookmark) => bookmark._ref !== tenant._id
      );
