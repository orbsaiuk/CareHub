...
export async function GET(request) {
  try {
    ...
    // Check cache for stats or events
    const cacheKey = stats
      ? `event-stats:${tenantId}`
      : `events:${tenantId}:${startDate || "all"}:${endDate || "all"}`;
    ...
    if (stats) {
      ...
      data = await writeClient.fetch(EVENT_STATS_QUERY, {
        tenantId,
        monthStart,
        monthEnd,
      });
    } else if (startDate && endDate) {
      data = await writeClient.fetch(EVENTS_BY_DATE_RANGE_QUERY, {
        tenantId,
        startDate,
        endDate,
      });
    } else {
      data = await writeClient.fetch(EVENTS_BY_TENANT_QUERY, {
        tenantId,
      });
    }
