...
export async function GET(req) {
  ...
  try {
    ...
    const contact = "dateshubuk@gmail.com";
    ...
    const nominatim = async (zoom) => {
      ...
      const url = `https://nominatim.openstreetmap.org/reverse?format=jsonv2&lat=${encodeURIComponent(
        lat
      )}&lon=${encodeURIComponent(lng)}&zoom=${zoom}&addressdetails=1&namedetails=1&email=${encodeURIComponent(
        contact
      )}`;
    ...
    for (const z of [18, 16, 14, 12, 10]) {
      try {
        ...
        console.error(`Nominatim zoom ${z} failed:`, err.message);
      }
    }
