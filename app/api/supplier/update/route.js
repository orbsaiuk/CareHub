...
import { ensureBusinessTypeRef } from "@/services/sanity/businessTypes";
...
export async function PATCH(request) {
  try {
    ...
    const allowed = {
      ...
      logo: rawUpdate.logo,
      businessType: rawUpdate.businessType,
      ...
      foundingYear: rawUpdate.foundingYear,
      registrationNumber: rawUpdate.registrationNumber,
    };
    const updates = Object.fromEntries(
      Object.entries(allowed).filter(([, v]) => typeof v !== "undefined")
    );

    // Convert businessType to proper reference if provided
    if (updates.businessType) {
      const businessTypeRef = await ensureBusinessTypeRef(
        updates.businessType,
        "supplier"
      );
      if (businessTypeRef) {
        updates.businessType = businessTypeRef;
      } else {
        delete updates.businessType; // Remove if invalid
      }
    }
