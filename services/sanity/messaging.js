...
import {
  ...
  TENANT_BY_TENANT_ID_QUERY,
  USER_ID_BY_CLERK_ID_QUERY,
  TENANT_ID_BY_KIND_AND_TENANT_ID_QUERY,
  UNREAD_MESSAGES_BY_CONVERSATION_AND_PARTICIPANT_QUERY,
...
export async function getOrCreateConversation({ participant1, participant2 }) {
  ...
  const existing = await writeClient.fetch(CONVERSATION_BY_PARTICIPANTS_QUERY, {
    p1: participant1,
    p2: participant2,
  });
...
export async function findOrCreateUserConversation({
  ...
  const user = await writeClient.fetch(USER_BY_CLERK_ID_QUERY, { userId });
  ...
  const company = await writeClient.fetch(TENANT_BY_TENANT_ID_QUERY, {
    kind: "company",
