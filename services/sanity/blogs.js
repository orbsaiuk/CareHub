...
import {
  ...
  PUBLISHED_BLOGS_BY_ROLE_QUERY,
  FEATURED_BLOGS_QUERY,
  BLOG_BY_SLUG_QUERY,
  BLOGS_BY_AUTHOR_QUERY,
  RECENT_BLOGS_QUERY,
  RECENT_BLOGS_BY_ROLE_QUERY,
  RELATED_BLOGS_QUERY,
  buildBlogsQuery,
  ...
  BLOG_BY_ID_QUERY,
  BLOGS_BY_TENANT_QUERY,
} from "@/sanity/queries/blogs";

export async function getPublishedBlogs(role = null) {
  try {
    // Always use role-filtered query to ensure proper visibility
    const effectiveRole = role || "user";
    return await sanityFetch(PUBLISHED_BLOGS_BY_ROLE_QUERY, { role: effectiveRole });
  } catch (error) {
    return [];
  }
}
...
export async function getFeaturedBlogs() {
  try {
    return await sanityFetch(FEATURED_BLOGS_QUERY);
  } catch (error) {
    return [];
  }
}
...
export async function getBlogBySlug(slug) {
  try {
    if (!slug) return null;
    return await sanityFetch(BLOG_BY_SLUG_QUERY, { slug });
  } catch (error) {
    return null;
  }
}
...
export async function getBlogById(id) {
  try {
    if (!id) return null;
    return await sanityFetch(BLOG_BY_ID_QUERY, { id });
  } catch (error) {
    return null;
  }
}
...
export async function getBlogsByAuthor(authorId) {
  try {
    if (!authorId) return [];
    return await sanityFetch(BLOGS_BY_AUTHOR_QUERY, { authorId });
  } catch (error) {
    return [];
  }
}

export async function getRecentBlogs(limit = 5, role = null) {
  try {
    // Always use role-filtered query to ensure proper visibility
    const effectiveRole = role || "user";
    return await sanityFetch(RECENT_BLOGS_BY_ROLE_QUERY, { limit, role: effectiveRole });
  } catch (error) {
    return [];
  }
}
...
export async function getRelatedBlogs(currentBlogId, authorId = null) {
  try {
    if (!currentBlogId) {
      ...
      return await sanityFetch(RECENT_BLOGS_QUERY, { limit: 5 });
    }

    // If we have an authorId, try to get blogs by the same author
    if (authorId) {
      const relatedBlogs = await sanityFetch(RELATED_BLOGS_QUERY, {
        currentBlogId,
        authorId,
      });
...
export async function getBlogsForTenant(tenantType, tenantId) {
  try {
    ...
    return await writeClient.fetch(BLOGS_BY_TENANT_QUERY, {
      filter,
      tenantId,
    });
  } catch (error) {
...
export async function getPopularBlogTags() {
  try {
    return await sanityFetch(POPULAR_BLOG_TAGS_QUERY);
  } catch (error) {
    return [];
  }
}
...
export async function getBlogArchive() {
  try {
    return await sanityFetch(BLOG_ARCHIVE_QUERY);
  } catch (error) {
    return [];
  }
}
