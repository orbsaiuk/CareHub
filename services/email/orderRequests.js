...
import { COMPANY_BY_TENANT_ID_FOR_EMAIL_QUERY } from "@/sanity/queries/company";
import { USER_BY_CLERK_ID_FOR_EMAIL_QUERY } from "@/sanity/queries/user";
    ...
    try {
        ...
        const html = buildBasicHtmlEmail(
            ...
            [
                ...
                "",
                "**تفاصيل طلبك:**",
                `• تفاصيل الطلب: ${orderRequest.orderDetails}`,
                `• عنوان التوصيل: ${orderRequest.deliveryAddress}`,
                orderRequest.deliveryDate ? `• تاريخ التوصيل: ${new Date(orderRequest.deliveryDate).toLocaleDateString("ar-EG")}` : "",
                orderRequest.phoneNumber ? `• رقم الهاتف: ${orderRequest.phoneNumber}` : "",
                "",
                "**الخطوات القادمة:**",
...
export async function sendOrderRequestNotificationToCompany(orderRequest) {
    try {
        const company = await writeClient.fetch(
            COMPANY_BY_TENANT_ID_FOR_EMAIL_QUERY,
        ...
        const subject = `🔔 طلب جديد من ${requesterName}`;
        const html = buildBasicHtmlEmail(
            `لديك طلب جديد`,
            [
                `عزيزنا ${company.name},`,
                ...
                `• تفاصيل الطلب: ${orderRequest.orderDetails}`,
                `• عنوان التوصيل: ${orderRequest.deliveryAddress}`,
                orderRequest.deliveryDate ? `• تاريخ التوصيل المطلوب: ${new Date(orderRequest.deliveryDate).toLocaleDateString("ar-EG")}` : "",
                orderRequest.phoneNumber ? `• رقم الهاتف: ${orderRequest.phoneNumber}` : "",
                "",
...
export async function sendOrderRequestResponseToCustomer(orderRequest) {
    try {
        ...
        const userDoc = await writeClient.fetch(USER_BY_CLERK_ID_FOR_EMAIL_QUERY, {
        ...
        if (isAccepted) {
            ...
            bodyLines.push(
                "**ملخص تفاصيل الطلب:**",
                `• تفاصيل الطلب: ${orderRequest.orderDetails || "غير محدد"}`,
                `• عنوان التوصيل: ${orderRequest.deliveryAddress || "غير محدد"}`,
                orderRequest.deliveryDate ? `• تاريخ التوصيل: ${new Date(orderRequest.deliveryDate).toLocaleDateString("ar-EG")}` : "",
                orderRequest.phoneNumber ? `• رقم الهاتف: ${orderRequest.phoneNumber}` : "",
                ""
            );
