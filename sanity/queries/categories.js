...
*[_type=="category"] | order(coalesce(displayOrder, 999) asc, title asc){
  _id,
  title,
  "slug": slug.current,
  icon,
...
*[_type=="category" && ((tenantType == $tenantType && tenantId == $tenantId) || (!defined(tenantType) && !defined(tenantId)))] | order(coalesce(displayOrder, 999) asc, title asc){
  _id,
  title,
  "slug": slug.current,
  icon,
...
export const CATEGORIES_BY_TITLES_QUERY = `
